...
export const useStockMutations = ({ refetchQueries, setLoadingRows }: MutationHooks) => {
  ...
  const exportMutation = createGenericMutation(
    {
      mutationFn: (data: TauriTypes.StockRivenControllerGetListParams) => api.stock_riven.exportJson(data),
  ...
  const updateMultipleMutation = createGenericMutation(
    {
      ...
      successKey: "update_stock_riven",
      errorKey: "update_stock_riven",
      isMultiple: (variables: { ids: number[]; input: TauriTypes.UpdateStockRiven }) => variables.ids.length > 1,
      getLoadingId: (variables: { ids: number[]; input: TauriTypes.UpdateStockRiven }) => variables.ids.map((id) => `${id}`),
